		...
		if('message' == message.type && message.text != null && message.subtype != "bot_message")
		{
			...
			{	// The message matched self.config.pattern
				...
				else if(!username_matches || !self.config.specify_username)
				{
					...
					self.slacker.send('users.info',	{ user: message.user }, function(value)
					{ 
						var guessed_user = fetchFromObject(value, self.config.user_guess_property);
						if(self.config.user_guess_search)
						{
							guessed_user = guessed_user.replace(self.config.user_guess_search, self.config.user_guess_replace);
						}
						sendLink(self, message, msg + guessed_user) });
				}
				else
				{
					sendHelp(message);
